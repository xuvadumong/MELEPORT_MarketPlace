...
impl Contract {
    ...
    pub fn get_sale_for_swap(&self, account_id: AccountId) -> (String, String) {
        let owner_sales = self.get_sales_by_owner_id(account_id, Some(U128(0)), Some(100));
        for sale in owner_sales.iter() {
            if sale.is_swap {
                return (sale.nft_contract_id.clone(), sale.token_id.clone());
            }
        }
        ("".to_string(), "".to_string())
    }
